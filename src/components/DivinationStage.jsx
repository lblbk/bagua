    ...
    {['manual', 'semi', 'full'].map((id) => (
      <button
        ...
        className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${
          selectedMode === id ? 'bg-slate-700 dark:bg-indigo-600 text-white shadow-md' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50'
        } ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
      >
        {id === 'manual' ? '手动' : id === 'semi' ? '半自动' : '全自动'}
      </button>
    ))}
  </div>
);
...
const ControlPanel = ({ status, selectedMode, isAutoSequence, historyCount, onMainAction }) => {
  ...
  const config = {
    idle: {
      text: { manual: '开始转动', semi: '掷一次', full: '开始排盘' }[selectedMode],
      color: 'bg-green-500 hover:bg-green-600 dark:bg-emerald-600 dark:hover:bg-emerald-700'
    },
    spinning: {
      text: selectedMode === 'manual' ? '点击停止' : '转动中...',
      color: 'bg-red-500 hover:bg-red-600 dark:bg-rose-700 dark:hover:bg-rose-800'
    },
    stopping: { text: '正在减速...', color: 'bg-gray-400 dark:bg-slate-700' },
    finished: { text: '占卜已圆满', color: 'bg-slate-100 dark:bg-slate-800 !text-slate-400 cursor-not-allowed' }
  };

  const current = config[status];
  const isDisabled = status === 'finished' || status === 'stopping' || (selectedMode === 'full' && isAutoSequence);
  const btnText = isAutoSequence && status !== 'finished' ? `自动排盘中... (${historyCount + 1}/6)` : current.text;
  ...
  return (
    <div className="w-full h-14">
      <button
        ...
        className={`w-full py-3 rounded-xl text-lg font-bold text-white shadow-lg transition-all active:scale-95 ${current.color} ${
          isDisabled && status !== 'finished' ? 'opacity-80 cursor-not-allowed' : ''
        }`}
      >
        {btnText}
      </button>
